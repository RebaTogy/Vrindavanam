...
function ensureProductVarietyDetailButtons() {
    document.querySelectorAll('.product-variety-card').forEach(card => {
        ...
        if (!productPrice) {
          ...
          // After updating cards, initialize detail page if on a detail page
          if (document.body.classList.contains('product-detail-page')) {
            const params = new URLSearchParams(window.location.search);
            const key = normalizeProductDetailKey(params.get('product'));
            const productDetail = productDetailCatalog[key] || productDetailCatalog['masala-cardamom'];
            if (productDetail) {
              const imgEl = document.getElementById('detailImage');
              if (imgEl) imgEl.src = productDetail.image;
              const nameEl = document.getElementById('detailName');
              if (nameEl) nameEl.textContent = productDetail.name;
              const descEl = document.getElementById('detailDescription');
              if (descEl) descEl.textContent = productDetail.description;
              const priceEl = document.getElementById('detailPrice');
              if (priceEl) priceEl.innerHTML = '₹' + Number(productDetail.price).toLocaleString('en-IN');
            }
          }
            }
        }
...
function renderProductDetailPage() {
    ...
    if (!select && buyButton) {
        ...
        buyButton.insertAdjacentElement('beforebegin', select);        if (select && product && Array.isArray(productsData) && productsData.length) {
            // Find the matching product from the API data to compute accurate weights and prices
            const apiProduct = productsData.find(p => p.product_name === product.name);
            if (apiProduct) {
                const originalWeight = parseFloat(apiProduct.weight.replace(/[^\d.]/g, ''));
                const originalPrice = parseFloat(apiProduct.price);
                // Define typical weight options (in grams)
                // Use the same weight options as the variety cards: 50g, 100g, 250g, 500g, 1kg
                const weightValues = [50, 100, 250, 500, 1000]; // grams
                weightValues.forEach((w, idx) => {
                    const calculatedPrice = Math.round((originalPrice / originalWeight) * w);
                    const optEl = document.createElement('option');
                    optEl.value = calculatedPrice;
                    const weightLabel = w >= 1000 ? (w / 1000) + 'kg' : w + 'gm';
                    optEl.textContent = `${weightLabel} — ${formatProductPagePrice(calculatedPrice)}`;
